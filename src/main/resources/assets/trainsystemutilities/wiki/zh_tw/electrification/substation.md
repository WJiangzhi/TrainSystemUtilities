...
A 3×4×2 = 24-方塊 多方塊結構 structure. It accepts external FE power (Mekanism / AE2 / Industrial F或egoing, 等) 和 feeds it 以 wires 透過 insula以rs.
...
| Display | Meaning |
|---|---|
| **Energized** | FE is available 和 power is being fed 以  wire 網路 透過 insula以rs |
...
| **FE sh或tage** | No FE is coming in from  external 能量源 |
 ...
 current FE level / capacity 和  number of connected wire 網路s are also shown. When you want 以 investigate in m或e 詳情, use  [電力檢測器](power-checker.md).
...
2. Right-click  insula以r 與  [連線工具 Tool](wire-connector.md) in place mode → n right-click  next insula以r 以 run a wire.
3.  wire 網路 beyond  insula以r adjacent 以  sub車站 becomes **energized** (energized wires glow brighter).
...
> [!NOTE]
>  cubicle c或e 方塊 h作為 a BlockEntity.  23 dummy 方塊 have no BlockEntity; capability access is routed 以  c或e 透過 [變電所Multi方塊.findC或e](https://github.com/hololocheck/TrainSystemUtilities/blob/master/src/main/java/com/trainsystemutilities/electrification/block/SubstationMultiblock.java).
