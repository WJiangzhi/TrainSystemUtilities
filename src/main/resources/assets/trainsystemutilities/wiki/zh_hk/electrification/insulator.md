...
一種支撐點，物理上固定接觸網，並在電氣上將其連接到方塊網絡（變電所 / 另一個絕緣子）。
變電所本身無法直接外露接觸網，必須始終通過絕緣子連接。
...
| 功能 | 説明 |
|---|---|
| **接觸網支撐點** | 用[連接工具](wire-connector.md)點擊的端點（= 接線針） |
| **電氣中繼** | 從變電所接收 FE，並**通電**到與之相連的每一段接觸網 |
| **隔離** | 在需要連接不同電網的接觸網時作為邊界（為未來擴展預留） |

## 如何放置

從物品欄手持絕緣子，**右鍵點擊你想要放置的面**。

- **絕緣子會從你右鍵點擊的那一面生長出來。** 右鍵地板（方塊頂面）它會朝上立起；右鍵天花板（底面）它會向下懸掛；右鍵牆壁（側面）它會向側方伸出。
...
絕緣子作為邊（= 接觸網連接）被記錄在 **WireNetworkSavedData** 中。
...
用[電力檢測器](power-checker.md)右鍵絕緣子，可在聊天欄顯示該絕緣子上連接的接觸網總數以及當前通電的數量。

## 相關

- [接觸網 / 連接工具](wire-connector.md) — 在絕緣子之間架設接觸網的工具（右鍵絕緣子）
