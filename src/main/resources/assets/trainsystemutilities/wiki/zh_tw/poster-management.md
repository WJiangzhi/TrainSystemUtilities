...
| Open animation settings | **Click  "♫ 動畫" but以n** → [動畫設定](poster-management/animation.md) popup |
...
| Ordering | Re或der 透過 swap in  list (帶有nimation) |
| Au以 / single switch | Slideshow 或 single-image 顯示 |
| FIT / COVER | How  image fits 與in  frame (cropped vs letterboxed) |
| Moni以r link | Display on linked 顯示器 透過  [顯示器連線卡](tools/monitor-link-card.md) |
