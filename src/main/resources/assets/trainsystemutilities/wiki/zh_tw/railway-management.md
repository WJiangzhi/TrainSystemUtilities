...
A 顯示 板 方塊 placed 在…上 車站 月臺. It 自動顯示s 已停靠 / 即將到站的列車 和 integrates 與 顯示器, col或 settings, 和廣播 (SAS).

[[TOC]]

## How 以 open

1. Place  **鐵路管理方塊** 在…上 車站 月臺. Just 例如 Create's 車站 方塊, you place it on以 a 軌道 by **right-clicking a 軌道** you have laid.
2. **Right-click**  placed 方塊 以 open  GUI.
3. Which 車站 it 顯示 is decided by linking this 方塊 以  [管理用計算機](management-computer/overview.md) 帶有 [儲存卡](tools/memory-card.md).
...
| Open 顯示器 settings | **Click  "⚙ Settings" but以n** on  顯示器 row → [顯示器設定](railway-management/settings.md) popup |
| Open col或 settings | **Click  "▒ Col或" but以n** on  顯示器 row → [顏色設定](railway-management/color.md) popup |
| Open announcement / 月臺-do或 settings | **Click  "功能 ▼" but以n** on  顯示器 row → from  list  appears, **click "廣播" 或 "Plat形式 Do或"** |
...
> [!TIP]
> This 方塊 is a "顯示-only" 板 placed 在…上 車站 月臺. To adjust numbers 或 col或s in bulk, operate from  popup side ([顯示器設定](railway-management/settings.md) / [顏色設定](railway-management/color.md)). Numbers inside  popup are incre作為ed/decre作為ed by **hovering over  value 和 using  mouse wheel** (re are no ＋ / − but以ns).
...
| Col或 自定義 | Change  col或 of each text element 透過  [col或 popup](railway-management/color.md) |
| 廣播 (SAS) | Departure melodies / announcements 透過 [SpatialAudioSystem](railway-management/announcement.md) 整合 |
| Batch apply | Apply settings at once 以 全部 方塊 in  same 網路 |
...
| Popup | Content |
|---|---|
| [顯示器設定](railway-management/settings.md) | Font size / 軌道 position / clock 顯示 / batch apply |
| [顏色設定](railway-management/color.md) | Col或s of 10 text elements (arrTime, depTime, trainName, 等) |
| [廣播設定](railway-management/announcement.md) | SAS-integrated entry management (條件al triggers) |

## Related

- [管理用計算機 概述](management-computer/overview.md) — 總覽 management of 全部 車站 和 列車
- [顯示器連線卡](tools/monitor-link-card.md) — link 與 nearby 顯示器
