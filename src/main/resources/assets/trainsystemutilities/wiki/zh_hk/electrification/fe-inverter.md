...
一種放置在列車車廂上的 FE 緩衝方塊。它存儲受電弓收集的電流，並向其他模組的機器供給 FE。

[[TOC]]

## 安裝

FE 逆變器是一個 **3 方塊聯動設備**。手持並**右鍵**點擊你想要的位置時，點擊位置會成為中心，並在其前後自動放置 3 方塊。

1. 手持逆變器並**右鍵**點擊想要的位置，例如車廂地板下方。
2. 朝向方向的前後需要**各 3 方塊的空餘空間**。沒有空餘空間則無法放置。
3. 破壞 3 個聯動方塊中的任意一個，3 個會同時被移除。
4. 即使受電弓安裝在不同的車廂上，它也會與同一列車上的 FE 逆變器自動共享電力。
5. 逆變器通過 IEnergyStorage 能力向相鄰方塊輸出 FE。
...
- 緩衝聚合：3 台 = 3,000,000 FE 容量

## 兼容的外部模組

| 模組 | 連接方式 |
|---|---|
| **Mekanism** | 通過 Universal Cable / Ultimate Energy Cube 等相鄰連接 |
| **Applied Energistics 2** | 通過 Energy Cell / Energy Acceptor 連接 |
| **Industrial Foregoing** | 通過 Power Conduit 連接 |
| **Create**（經電動引擎） | 相鄰進行 FE → 旋轉力轉換 |
| 其他兼容 FE 的模組 | 均可通過 IEnergyStorage 連接 |

## GUI / 狀態檢查

FE 逆變器本體沒有專用 GUI（它只是個簡單的緩衝方塊）。可以通過以下 2 種方式檢查其狀態。

- **空手右鍵已放置的逆變器** → 在聊天欄顯示該單體的 FE 電量 / 容量。
- **管理計算機 > 列車詳情 > 電氣化詳情** → 一覽整列車的儲能（[電氣化詳情彈窗](../management-computer/trains.md)）。
- **用[電力檢測器](power-checker.md)右鍵** → 放置時顯示單體信息，已編入列車時顯示所屬列車的總儲能。

> [!TIP]
> 即便列車在長距離路段中離開區塊，FE 逆變器的緩衝電量也會保留在服務器上（= 與區塊加載無關）。
