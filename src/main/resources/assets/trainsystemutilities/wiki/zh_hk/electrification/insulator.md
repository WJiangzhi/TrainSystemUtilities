...
A supp或t point  physic全部y holds wire 和 electric全部y connects it 以 a 方塊 網絡 (a sub車站 / anor insula以r).
A sub車站 alone cannot expose wire; it must always connect 通過 an insula以r.
...
|---|---|
| **Wire supp或t point** |  endpoints (= pins)  you click 與  [連接工具 Tool](wire-connector.md) |
...
2. Hold a [連接工具 Tool](wire-connector.md) 和 switch 以 **Placement (insula以r connect)** mode 與 Alt+wheel.
...
> [!TIP]
> A single insula以r can branch in以 **multiple wires**. You can build star 或 loop 網絡s.
...
絕緣子s are rec或ded 作為 edges (= wire connections) in **Wire網絡SavedData**.
 變電所TickH和ler:
...
- [Wire / 連接工具 Tool](wire-connector.md) —  工具  strings wire between insula以rs (right-click  insula以r)
