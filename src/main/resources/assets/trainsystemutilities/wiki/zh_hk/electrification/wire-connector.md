...
一種用於在兩點之間架設接觸網的專用工具。支持 5 種內置樣式 + 保存任意自定義預設。

[[TOC]]

> [!IMPORTANT]
> 接觸網**不能**直接架設到支柱 / 桁架上。必須升起兩個**接觸網絕緣子**，並在**絕緣子與絕緣子之間**架設接觸網。支柱和桁架是承載絕緣子的底座。請先閲讀[接觸網絕緣子](insulator.md)並準備好支撐點。

## 此工具的兩種模式

通過**按住 Alt 鍵並滾動鼠標滾輪**，連接工具可在兩種模式間切換（手持時，當前模式會顯示在物品欄上方）。

| 模式 | 物品欄標籤 | 模式作用 |
|---|---|---|
| **放置（絕緣子連接）** | `Placement Mode` | 右鍵絕緣子以架設接觸網（見下方"如何架設接觸網"） |
| **GUI（樣式選擇）** | `GUI Mode` | 右鍵打開接觸網設置界面，編輯樣式或剩餘量 |

> [!NOTE]
> Alt+滾輪切換的是**模式，而非接觸網樣式類型**。樣式選擇（SIMPLE / TWO_TIER 等）在"GUI 模式"下右鍵打開的設置界面內進行。

## 如何架設接觸網（放置模式）

1. 首先在想要支撐的兩個點放置**接觸網絕緣子**（右鍵放置在地板、支柱、桁架側面等上）。
2. 手持連接工具，用 Alt+滾輪切換到**放置（絕緣子連接）** 模式。
...
生存模式下，架設接觸網會按架設距離（m）消耗工具的"接觸網剩餘量"。可在工具提示和 GUI 模式界面上方的量表中查看剩餘量。

1. 準備一個**接觸網線軸**（`Wire Spool` 物品；1 個提供 100 m）。
2. 用 Alt+滾輪切換到 **GUI 模式** 並右鍵 → 打開接觸網設置界面。
3. 將線軸放入屏幕左側的 **"Wire Load"** 槽位，點擊 **"Refill Wire"** 按鈕即可填充工具的內部儲罐（最大 6400 m）。
...
2. **右鍵**點擊工具（對方塊或向空中） → 打開接觸網設置界面。
3. 在左側面板選擇樣式。
4. 在右側面板查看 / 編輯參數。
5. 用屏幕底部的 **"Apply"** 確認設置（會出現確認對話框）。
...
開啓後，單線中央會下垂以產生裝飾效果（= 營造舊時代電氣化的感覺）。

## 預設保存

編輯 CUSTOM 時，點擊 "Save Preset" → 在對話框中輸入預設名稱 → Enter 保存。
保存後會添加到左側面板的磁貼列表中，可立即選用 → 應用。
...
"Apply" 按鈕保存當前設置。接着，**在放置模式下右鍵兩個絕緣子**即可用該樣式架設新的接觸網。
應用前會顯示確認對話框。
...
- [自定義接觸網設計](custom-wire.md) — CUSTOM 詳細參數
