...
[連接工具](wire-connector.md)的 `CUSTOM` 模式所使用的自由參數調節 + 預設保存系統。

[[TOC]]

## 如何打開

1. 手持[連接工具](wire-connector.md)，通過 **Alt+滾輪** 切換到 **GUI（樣式選擇）** 模式。
2. **右鍵**點擊工具，打開接觸網設置界面。
3. 在左側面板的樣式磁貼中選擇 **CUSTOM**，右側面板會切換到下方的編輯模式。

## 選擇 CUSTOM 時的編輯面板

選擇 CUSTOM 磁貼後，右側面板會切換到編輯模式，你可以用**鼠標滾輪**調整以下數值：
...
"**Save Preset**" 按鈕僅在編輯 CUSTOM 時激活。

![](bws:trainsystemutilities:wiki/screens/wire-connector-preset-save__ja_jp.png)

1. 點擊 "Save Preset" 按鈕 → 出現保存對話框
2. 輸入預設名稱（默認：`PresetN`）
3. **Enter** 保存 / **Esc** 取消

保存後，預設會被添加到左側面板的磁貼列表（= 樣式磁貼）中，可以立即選用。
...
- 左鍵：應用預設 → 自定義參數被**鎖定**為預設值（滾輪禁用）
- 回到編輯模式：從磁貼列表中選擇 **CUSTOM**

> [!TIP]
> 預設與正在編輯的數值是分開管理的。已保存的預設無法編輯，若要修改某個預設，請新建一個並刪除舊的。

## 與下垂模式的關係

下垂模式**僅適用於 SIMPLE**，不能與 CUSTOM 一起使用。
即便 CUSTOM 設計設置為單層 + 細線，也不會下垂（= 僅呈直線）。

## 與列車預設的交互

用自定義接觸網鋪設的路段不會保存在列車預設側（= 接觸網屬於世界側實體）。
若要在其他世界鋪設外觀相同的接觸網，請共享連接工具的預設 JSON。

## 相關

- [接觸網 / 連接工具](wire-connector.md) — 基本操作與內置樣式
- [受電弓](pantograph.md)
- [箱式變電所](substation.md)
