---
title: 線路編號標籤頁
id: management-computer/line-symbols
tags: [management-computer, line-symbol]
---

# 線路編號標籤頁

![](bws:trainsystemutilities:wiki/screens/management-computer__symbol__ja_jp.png)

管理用計算機的"線路編號"標籤頁。創建、編輯和刪除線路編號（如 `JA`、`JB`、`M01`）。

[[TOC]]

## 如何打開

1. **放置**管理用計算機方塊並**右鍵點擊**它以打開界面。
2. **點擊**左上角下拉菜單並選擇**"Ⓜ 線路編號"**。
3. 當編號無法全部顯示時，在列表上轉動**鼠標滾輪**滾動。

## 什麼是線路編號？

分配給每個車站和列車的標識符。例如：山手線 = `JY`，中央線 = `JC`。
在[車站標籤頁](stations.md)中分配給車站 → 顯示在[線路圖](route-map.md)和[鐵路管理方塊](../railway-management.md)上。

## 顯示內容

| 列 | 內容 |
|---|---|
| 編號文字 | 2-3 字符的線路縮寫（如 `JA`） |
| 顏色 | 背景 / 文字顏色 |
| 形狀 | 圓形 / 圓角方形 / 六邊形 等 |
| 使用它的車站 | 該編號已分配給的車站數量 |

## 控件

| 操作 | 方法 | 結果 |
|---|---|---|
| 新建 | **點擊右上角的"＋ 新建"按鈕** | 打開[編號編輯器](symbol-editor.md)以創建新編號 |
| 編輯 | 在列表中**（左鍵）點擊編號磁貼** | 在[編號編輯器](symbol-editor.md)中編輯該編號 |
| 刪除 | 在列表中**右鍵點擊編號磁貼** | 出現刪除確認（[見下文](#記號削除時の挙動)） |

> [!TIP]
> 將鼠標懸停在編號磁貼上會顯示提示"左鍵：編輯 / 右鍵：刪除"。

## 編號刪除行為 {#記號削除時の挙動}

**右鍵點擊**編號磁貼會顯示**刪除確認**。**點擊"🗑 刪除"**確認；直接關閉則保留該編號。

刪除正在使用中的編號會從所有已分配該編號的車站清除編號。
請在[車站標籤頁](stations.md)中重新分配。

## 相關

- [編號編輯器](symbol-editor.md)
- [車站標籤頁](stations.md)
- [線路圖](route-map.md)
- [鐵路管理方塊](../railway-management.md)
