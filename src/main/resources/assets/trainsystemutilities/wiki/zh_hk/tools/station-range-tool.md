...
一種通過指定兩點來將一片車站區域註冊為**車站組**的工具。
它可將多個鐵路管理方塊歸併為一個車站，從而一次性應用通用設置（顏色 / 設置 / 廣播）。

[[TOC]]

## 模式切換

此工具用 **Alt + 鼠標滾輪**在三種模式間切換（手持時，當前模式會顯示在快捷欄上方）。

| 模式 | 行為 |
|---|---|
| 選擇（默認） | 左鍵點擊範圍的兩個角，創建車站組 |
| GUI | 右鍵打開車站組管理 GUI |
| 顯示 | 顯示世界中已有車站組的輪廓 |

## 用法（選擇模式）

1. 手持工具並**左鍵**點擊車站區域的第一個角。
2. **左鍵**點擊對角的另一個角。
3. 右鍵工具 → 彈出車站組保存 GUI。
4. 輸入車站組名稱 → 回車保存。
...
用 **Alt + 鼠標滾輪**將工具切換到 **GUI** 模式，然後**右鍵**打開管理 GUI，在其中可對已保存的車站組進行：
- 重命名
- 刪除（帶確認對話框）
- 查看成員車站

> [!NOTE]
> 車站組管理沒有命令，全部通過此工具的 GUI 模式完成。
...
- [鐵路管理方塊批量應用](../railway-management/settings.md#batch-apply) 一次性為同一組內的全部成員應用設置
- [SAS 廣播](../railway-management/announcement.md) 的共享目的地（share）
- [自動售票機](../structure/ticket-vending-machine.md) 的**候選售票車站**（從[車票標籤頁](../management-computer/tickets.md)中的車站組裏選取）
- 識別**自動檢票閘機**所指向的車站
- [換乘查詢終端](transit-terminal.md) **路徑檢索**的目標（以車站組作為起訖點進行檢索）

> [!NOTE]
> 自動售票機、自動檢票閘機和換乘檢索都**以本工具創建的車站組作為共享數據引用。** 在使用它們之前，請先將目標車站註冊為車站組。
